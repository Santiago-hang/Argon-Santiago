	...
	<div class="post-content" id="post_content">
		...
		<?php
				...
				foreach ($posts as $post){
					...
					$post_type_label = ($post -> post_type === 'shuoshuo') ? __('说说', 'argon') : __('文章', 'argon');
					$entry_title = get_the_title($post);
					if ($entry_title === ''){
						$entry_title = wp_trim_words(wp_strip_all_tags($post -> post_content), 40, '…');
					}
					if ($entry_title === ''){
						$entry_title = __('（说说）', 'argon');
					}
					if ($year != $last_year){
					...
					<div class='argon-timeline-node archive-timeline-entry archive-timeline-entry-<?php echo esc_attr($post -> post_type); ?>'>
						...
						<div class='argon-timeline-card card bg-gradient-secondary archive-timeline-title'>
							<a href="<?php echo esc_url(get_permalink($post)); ?>"><?php echo esc_html($entry_title); ?></a>
						</div>
					</div>
					<?php
				}
