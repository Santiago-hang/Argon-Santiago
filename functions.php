...
if (version_compare( $GLOBALS['wp_version'], '4.4-alpha', '<' )) {
	echo "<div style='background: #5e72e4;color: #fff;font-size: 30px;padding: 50px 30px;position: fixed;width: 100%;left: 0;right: 0;bottom: 0;z-index: 2147483647;'>" . __("Argon-Santiago 主题不支持 Wordpress 4.4 以下版本，请更新 Wordpress", 'argon') . "</div>";
}
...
switch ($argon_update_source) {
    ...
    default:
		$argonThemeUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
			'https://raw.githubusercontent.com/Santiago-hang/Argon-Santiago/main/info.json',
...
// 已注释：原作者已不更新，API 可能已失效，且使用明文 HTTP
...
function argon_widgets_init() {
	...
	register_sidebar(
		array(
			...
			'id'            => 'rightbar-tools',
			'description'   => __( '右侧栏小工具 (在 "Argon-Santiago 主题选项" 中选择 "三栏布局" 才会显示)', 'argon'),
...
function set_user_token_cookie(){
	if (!isset($_COOKIE["argon_user_token"]) || strlen($_COOKIE["argon_user_token"]) != 32){
		...
		setcookie("argon_user_token", $newToken, [
...
function argon_get_visitor_ua_display($userAgent = null){
	...
	$platform_icon = ($platform && isset($GLOBALS['UA_ICON'][$platform]))
		? $GLOBALS['UA_ICON'][$platform]
		: $GLOBALS['UA_ICON']['Unknown'];
	$browser_icon = ($browser && isset($GLOBALS['UA_ICON'][$browser]))
		? $GLOBALS['UA_ICON'][$browser]
		: $GLOBALS['UA_ICON']['Unknown'];
	return array(
		'platform' => $platform,
...
function parse_ua_and_icon($userAgent){
	...
	$ua = argon_get_visitor_ua_display($userAgent);
	$out = "<div class='comment-useragent'>";
	if (isset($argon_comment_show_ua['platform']) && $argon_comment_show_ua['platform'] == true){
		$out .= $ua['platform_icon'] . " ";
		if ($ua['platform']){
			$out .= esc_html($ua['platform']);
			if ($ua['platform_version']){
				$out .= esc_html($ua['platform_version']);
			}
		}
	}
	if (isset($argon_comment_show_ua['browser']) && $argon_comment_show_ua['browser'] == true){
		$out .= " " . $ua['browser_icon'];
		if ($ua['browser']){
			$out .= " " . esc_html($ua['browser']);
		}
		if (isset($argon_comment_show_ua['version']) && $argon_comment_show_ua['version'] == true && $ua['version']){
			$out .= " " . esc_html($ua['version']);
		}
	}
	$out .= "</div>";
...
//SMTP配置
function argon_smtp_config($phpmailer) {
    $phpmailer->isSMTP();
    $phpmailer->Host = defined('SMTP_HOST') ? SMTP_HOST : 'smtp.163.com';
    ...
    $phpmailer->From = defined('SMTP_FROM') ? SMTP_FROM : $phpmailer->Username;
    $phpmailer->FromName = defined('SMTP_FROMNAME') ? SMTP_FROMNAME : get_bloginfo('name');
}
add_action('phpmailer_init', 'argon_smtp_config');
...
function user_can_view_comment($id){
	...
	if (isset($_COOKIE['argon_user_token']) && hash_equals(get_comment_meta($id, "private_mode", true), $_COOKIE['argon_user_token'])){
		return true;
	}
	return false;
}
...
function get_comment_edit_history(){
	if (!check_ajax_referer('argon_ajax_nonce', 'nonce', false)){
...
function upvote_comment(){
	...
	setcookie('argon_comment_upvoted', $upvotedList . $ID . ",", [
			...
			<div class="comment-item-title">
				<div class="comment-name">
					...
					<?php
						...
						// 2. IP 属地（仅读评论 meta，不在列表页同步请求外网 API）
						$ip_loc = get_comment_meta(get_comment_ID(), 'ip_location', true);
						
						// 如果查到了数据，就在设备信息后方显示（样式与 comment-useragent 一致）
						if (!empty($ip_loc) && $ip_loc !== '未知(API错误)') {
							echo '<span class="comment-useragent"><i class="fa fa-map-marker text-danger" aria-hidden="true"></i> IP属地 ' . esc_html($ip_loc) . '</span>';
						}
					?>
				</div>
...
//评论邮箱是否可用于邮件提醒（有效邮箱或 QQ 号）
function argon_comment_email_can_mailnotice($email){
	...
	if (get_option('argon_comment_enable_qq_avatar') == 'true' && function_exists('check_qqnumber') && check_qqnumber($email)){
		return true;
	}
	return false;
}
...
function comment_mail_notify($comment){
	...
	if (get_comment_meta($parentID, "enable_mailnotice", true) == "true"){
		if (argon_comment_email_can_mailnotice($parentEmail)){
...
function post_comment_updatemetas($id){
	...
	//是否启用邮件通知（须勾选且留有可投递的邮箱或 QQ）
	if ($_POST['enable_mailnotice'] == 'true' && get_option("argon_comment_allow_mailnotice") == "true" && argon_comment_email_can_mailnotice($comment -> comment_author_email)){
...
//QQ Avatar 获取
function get_avatar_by_qqnumber($avatar, $comment_or_email = null){
	$comment = null;
	if (is_object($comment_or_email) && isset($comment_or_email -> comment_ID)){
		$comment = $comment_or_email;
	} elseif (is_numeric($comment_or_email)){
		$comment = get_comment($comment_or_email);
	} else {
		global $comment;
	}
	...
	if (empty($qqnumber)){
		if (strpos($email, '@avatarqq.com') !== false){
			$qqnumber = str_replace('@avatarqq.com', '', $email);
		}
	}
	if (!empty($qqnumber)){
		preg_match_all('/width=\'(.*?)\'/', $avatar, $preg_res);
		$size = $preg_res[1][0];
		$new_avatar = "<img src='https://q1.qlogo.cn/g?b=qq&s=640&nk=" . $qqnumber ."' class='avatar avatar-" . $size . " photo' width='" . $size . "' height='" . $size . "'>";
		return $new_avatar;
	}
	return $avatar;
}
...
function get_banner_background_url(){
	...
	if ($url == "--bing--"){
		...
		if ($now - $lastUpdated < 3600){
			...
			$response = wp_remote_get('https://www.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1', array('timeout' => 5));
...
function upvote_shuoshuo(){
	...
	setcookie('argon_shuoshuo_upvoted', $upvotedList . $ID . ",", [
		'expires' => time() + 3153600000,
		'path' => '/',
		'httponly' => true,
		'secure' => is_ssl(),
...
function alert_footer_copyright_changed(){ ?>
	<div class='notice notice-warning is-dismissible'>
		<p><?php _e("警告：你可能修改了 Argon-Santiago 主题页脚的版权声明，Argon-Santiago 主题要求你至少保留主题的 Github 链接或主题的发布文章链接。", 'argon');?></p>
	</div>
<?php }
function check_footer_copyright(){
	$footer = file_get_contents(get_theme_root() . "/" . wp_get_theme() -> template . "/footer.php");
	if ((strpos($footer, "github.com/Santiago-hang/Argon-Santiago") === false) && (strpos($footer, "yuhang.xyz") === false)){
...
function update_post_meta_ajax(){
	...
	// 权限检查：用户必须有编辑该文章的权限
	if (!current_user_can('edit_post', $post_id)) {
		exit(json_encode(array('status' => 'error', 'message' => 'Permission denied')));
	}

	// meta_key 白名单，防止修改任意字段
	...
	if (!in_array($meta_key, $allowed_meta_keys, true)) {
		exit(json_encode(array('status' => 'error', 'message' => 'Invalid meta key')));
	}
...
function shortcode_github($attr,$content=""){
	...
	// SSRF 防护：验证 author 和 project 只含合法字符
	if (!preg_match('/^[a-zA-Z0-9._-]+$/', $author) || !preg_match('/^[a-zA-Z0-9._-]+$/', $project)){
		return '';
	}
...
function themeoptions_admin_menu(){
	/*后台管理面板侧栏添加选项*/
	add_menu_page(__("Argon-Santiago 主题设置", 'argon'), __("Argon-Santiago 主题选项", 'argon'), 'edit_theme_options', basename(__FILE__), 'themeoptions_page');
}
...
// ==========================================
// 访客 IP / IP 属地（评论与页脚共用，瞬态缓存 24 小时）
// ==========================================
function argon_get_visitor_ip(){
...
function argon_filter_comment_user_ip($ip){
	return argon_get_visitor_ip();
}
add_filter('pre_comment_user_ip', 'argon_filter_comment_user_ip');
function argon_is_private_ip($ip){
	$ip = trim((string) $ip);
...
function argon_get_cached_ip_location($ip){
	$ip = trim((string) $ip);
	if ($ip === ''){
		return null;
	}
	if (argon_is_private_ip($ip)){
		return '局域网';
	}
	$cached = get_transient('argon_ip_loc_' . md5($ip));
	return ($cached !== false) ? $cached : null;
}
function argon_lookup_ip_location($ip, $use_transient = true){
	$ip = trim((string) $ip);
	if ($ip === ''){
		return '';
	}
	...
	$cache_key = 'argon_ip_loc_' . md5($ip);
	if ($use_transient){
...
function get_and_cache_ip_location($comment_id){
	$comment = get_comment($comment_id);
	if (!$comment){
		return '';
	}
	$existing = get_comment_meta($comment_id, 'ip_location', true);
	if ($existing !== ''){
		return $existing;
	}
...
function ajax_argon_footer_ip_location(){
	if (!check_ajax_referer('argon_ajax_nonce', 'nonce', false)){
		wp_send_json_error();
	}
	$ip = argon_get_visitor_ip();
	$location = argon_lookup_ip_location($ip);
	if ($location === '' || $location === '未知(API错误)'){
		wp_send_json_error();
	}
	wp_send_json_success(array('location' => $location));
}
add_action('wp_ajax_argon_footer_ip_location', 'ajax_argon_footer_ip_location');
add_action('wp_ajax_nopriv_argon_footer_ip_location', 'ajax_argon_footer_ip_location');
 
// ==========================================
// 访客统计：暂存访问数据，北京时间12:00和00:00写入数据库
// ==========================================

// 每次访问：存入暂存，不碰数据库
function argon_update_visitor_stats() {
    if (is_admin() || is_feed() || is_trackback()) {
        return;
    }

    $current_token = isset($_COOKIE['argon_user_token']) ? $_COOKIE['argon_user_token'] : '';
    if (empty($current_token)) {
        return;
    }

    $pending = get_transient('argon_visitor_pending');
    if ($pending === false) {
        $pending = array('visitors' => 0, 'views' => 0, 'tokens' => array());
    }

    $pending['views']++;

    if (!in_array($current_token, $pending['tokens'])) {
        $pending['tokens'][] = $current_token;
        $pending['visitors']++;
        if (count($pending['tokens']) > 5000) {
            $pending['tokens'] = array_slice($pending['tokens'], -2500);
        }
    }

    set_transient('argon_visitor_pending', $pending, 46800);
}

// 定时写入：把暂存数据倒进数据库，然后清空
function argon_flush_visitor_stats() {
    $pending = get_transient('argon_visitor_pending');
    if ($pending === false || ($pending['visitors'] == 0 && $pending['views'] == 0)) {
        return;
    }

    $stats = get_option('argon_visitor_stats', array(
        'total_visitors' => 0,
        'total_views' => 0
    ));

    $stats['total_visitors'] += $pending['visitors'];
    $stats['total_views'] += $pending['views'];

    update_option('argon_visitor_stats', $stats);
    delete_transient('argon_visitor_pending');
}
add_action('template_redirect', 'argon_update_visitor_stats');

// 定时任务：北京时间12:00和00:00各写入一次
add_action('init', 'argon_visitor_stats_check_cron');
function argon_visitor_stats_check_cron() {
    // 一次性迁移：清理旧版定时任务
    if (!get_option('argon_visitor_cron_v2_migrated')) {
        $old_timestamp = wp_next_scheduled('argon_visitor_stats_cron');
        if ($old_timestamp) {
            wp_unschedule_event($old_timestamp, 'argon_visitor_stats_cron');
        }
        update_option('argon_visitor_cron_v2_migrated', '1');
    }

    if (!wp_next_scheduled('argon_visitor_stats_cron')) {
        $beijing_offset = 8 * 3600;
        $seconds_since_midnight = (time() + $beijing_offset) % 86400;

        if ($seconds_since_midnight < 12 * 3600) {
            $next_flush = time() + (12 * 3600 - $seconds_since_midnight);
        } else {
            $next_flush = time() + (86400 - $seconds_since_midnight);
        }

        wp_schedule_event($next_flush, 'twice_daily_beijing', 'argon_visitor_stats_cron');
    }
}
add_action('argon_visitor_stats_cron', 'argon_flush_visitor_stats');

// 主题停用时清理定时任务
register_deactivation_hook(__FILE__, 'argon_visitor_stats_deactivate_cron');
function argon_visitor_stats_deactivate_cron() {
    $timestamp = wp_next_scheduled('argon_visitor_stats_cron');
    if ($timestamp) {
        wp_unschedule_event($timestamp, 'argon_visitor_stats_cron');
    }
}

// 添加12小时间隔
add_filter('cron_schedules', 'argon_add_cron_interval');
function argon_add_cron_interval($schedules) {
    $schedules['twice_daily_beijing'] = array(
        'interval' => 43200,
        'display' => '每12小时'
    );
    return $schedules;
...
// 读取统计（数据库 + 暂存，实时显示）
function argon_get_visitor_stats() {
    $pending = get_transient('argon_visitor_pending');
    $pending_visitors = ($pending !== false) ? $pending['visitors'] : 0;
    $pending_views = ($pending !== false) ? $pending['views'] : 0;
    ...
    return array(
        'total_visitors' => $stats['total_visitors'] + $pending_visitors,
        'total_views' => $stats['total_views'] + $pending_views
    );
}